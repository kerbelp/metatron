...
from metatron.storage.catalog import Catalog, CatalogEventStore, CatalogDecisionStore
from metatron.storage.transfer import import_catalog


def _seed(catalog, repo, pattern, *, actor):
    CatalogDecisionStore(catalog).add(Decision(repo=repo, pattern=pattern, scope="a",
...
def test_import_merges_repo_rows_and_preserves_attribution(tmp_path):
    ...
    assert counts["acme/app"]["decisions"] == 1 and counts["acme/app"]["events"] == 1

    patterns = {p.pattern for p in CatalogDecisionStore(dst).list(repo="acme/app")}
...
def test_import_is_idempotent(tmp_path):
    ...
    assert import_catalog(src, dst)["acme/app"]["decisions"] == 1
    # second import copies nothing new
    again = import_catalog(src, dst)
    assert again["acme/app"]["decisions"] == 0 and again["acme/app"]["events"] == 0
    assert len(CatalogDecisionStore(dst).list(repo="acme/app")) == 1
...
def test_import_cli_merges_a_handed_off_file(tmp_path, monkeypatch):
    ...
    _seed(employee, "acme/app", "shipped-decision", actor="Alice")
    ...
    assert [p.pattern for p in CatalogDecisionStore(curator).list(repo="acme/app")] == ["shipped-decision"]
