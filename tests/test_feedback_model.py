...
Build-version provenance (Event.version, Decision.created_version), the feedback
...
def test_new_decision_is_stamped_with_a_build_version():
    # created_version is auto-populated from the running build, non-empty.
    assert _decision().created_version
...
def test_decision_created_version_survives_a_store_round_trip():
    store = SQLiteDecisionStore(":memory:")
    p = _decision(created_version="abc1234")
    store.add(p)
...
def test_feedback_event_round_trips_with_ratings_and_missing_text():
    ...
    ev = Event(
        ...
        query_ref="query-123",
        helpful_decision_ids=["a", "b"],
        unhelpful_decision_ids=["c"],
    ...
    assert got.helpful_decision_ids == ["a", "b"]
    assert got.unhelpful_decision_ids == ["c"]
...
def test_event_store_get_returns_event_by_id_for_index_resolution():
    store = SQLiteEventStore(":memory:")
    ev = Event(repo="r", kind=EventKind.QUERY, decision_ids=["p1", "p2", "p3"])
    store.record(ev)
    fetched = store.get(ev.id)
    assert fetched is not None
    assert fetched.decision_ids == ["p1", "p2", "p3"]
...
def test_unhandled_feedback_excludes_handled_and_non_feedback():
    ...
    assert store.get(fb.id).decision_ids == ["cand1", "cand2"]
