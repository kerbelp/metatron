...
Answers "are feedback-born decisions better than ingest-born?" via curation
accept-rate and helpful-rate grouped by origin. Advisory only — reads counts and
feedback events, never mutates decisions.
"""
...
def _p(origin, status=Status.CANDIDATE, pattern="p") -> Decision:
...
def test_feedback_analytics_tallies_helpful_and_noise_by_origin():
    ...
    ev.record(Event(repo=REPO, kind=EventKind.FEEDBACK,
                    helpful_decision_ids=[fb.id], unhelpful_decision_ids=[boot.id]))
    ev.record(Event(repo=REPO, kind=EventKind.FEEDBACK, helpful_decision_ids=[fb.id]))
    ...
    per = {p["id"]: p for p in out["decisions"]}
...
def test_feedback_events_returns_raw_stream_newest_first():
    ...
    ev.record(Event(
        ...
        helpful_decision_ids=[helpful.id], unhelpful_decision_ids=[noisy.id],
...
def test_feedback_events_resolves_produced_candidates_when_handled():
    s, ev = SQLiteDecisionStore(":memory:"), SQLiteEventStore(":memory:")
    born = _p(Origin.AGENT_FEEDBACK, Status.CANDIDATE, pattern="refined decision")
    s.add(born)
    ...
    assert [p["pattern"] for p in e["produced"]] == ["refined decision"]
