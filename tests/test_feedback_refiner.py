...
LLM-assisted but human-gated — every decision it produces is a candidate of
...
def test_refiner_splits_a_gap_into_multiple_structured_candidates():
    ...
    decisions = FeedbackRefiner(StaticProvider(resp)).refine("big gap text", scope_hint="src/x")
    assert len(decisions) == 2
    assert all(p.origin is Origin.AGENT_FEEDBACK and p.status is Status.CANDIDATE for p in decisions)
    assert decisions[0].pattern.startswith("Mirror")
...
def test_refiner_defaults_scope_to_hint_and_skips_empty_patterns():
    ...
    decisions = FeedbackRefiner(StaticProvider(resp)).refine("g", scope_hint="src/db")
    assert [p.pattern for p in decisions] == ["Real rule"]
    assert decisions[0].scope == "src/db"
...
class FakeRefiner:
    ...
    def refine(self, gap: str, scope_hint: str = "", task: str = "") -> list[Decision]:
        return [
            Decision(repo="placeholder", pattern=f"p{i}:{gap[:6]}", scope=scope_hint,
...
def test_refine_feedback_creates_candidates_marks_handled_and_is_idempotent():
    ...
    assert (res.events_processed, res.decisions_created) == (2, 4)
