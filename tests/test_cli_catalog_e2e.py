...
def _decision(repo, pattern="p", status=Status.CANONICAL):
...
def test_db_flag_single_file_mode_resolves_lone_repo(tmp_path, monkeypatch):
    ...
    CatalogDecisionStore(cat).add(_decision("acme/app", "ship me", status=Status.CANDIDATE))
