/* ============================================================
   Shared UI primitives — badges, prior rows, detail drawer.
   ============================================================ */
...
/* ---------- prior row (browse / curation) ---------- */
function PriorRow({ prior, onOpen, style, children, columns = "1fr auto" }) {
  return (
    <div className="row enter" style={{ gridTemplateColumns: columns, cursor: onOpen ? "pointer" : "default", ...style }} onClick={onOpen ? () => onOpen(prior) : undefined}>
      <div style={{ minWidth: 0 }}>
        <div className="pattern" style={{ marginBottom: 9 }}>{prior.pattern}</div>
        <div style={{ display: "flex", alignItems: "center", gap: 14, flexWrap: "wrap" }}>
          <ScopeTag scope={prior.scope} />
          <OriginTag origin={prior.origin} />
          <Confidence level={prior.confidence} showLabel />
        </div>
      </div>
      {children || (
...
/* ---------- prior detail drawer ---------- */
function PriorDrawer({ prior, onClose, onApprove, onReject, busy }) {
  useEffect(() => {
    ...
    <div style={{ position: "fixed", inset: 0, zIndex: 120, display: "flex", justifyContent: "flex-end" }}>
      ...
      <aside style={{ position: "relative", width: "min(560px, 92vw)", height: "100%", background: "linear-gradient(180deg,#081512,#060f0d)", borderLeft: "1px solid var(--line-2)", boxShadow: "var(--shadow-deep)", overflowY: "auto", animation: "slidein .36s cubic-bezier(.2,.8,.3,1)" }}>
        ...
        <div style={{ position: "sticky", top: 0, zIndex: 2, padding: "20px 26px", background: "linear-gradient(180deg,#081512,rgba(8,21,18,.7))", backdropFilter: "blur(8px)", borderBottom: "1px solid var(--line)", display: "flex", alignItems: "center", gap: 12 }}>
          <StatusBadge status={prior.status} />
          {prior.triage && prior.triage !== "none" && prior.status === "candidate" && <TriageTag triage={prior.triage} />}
        ...
        <div style={{ padding: "26px 28px 40px" }}>
          ...
          <div style={{ fontSize: 20, lineHeight: 1.4, fontWeight: 400, color: "#eafff8", textWrap: "pretty" }}>{prior.pattern}</div>

          <div style={{ display: "flex", gap: 26, margin: "26px 0", flexWrap: "wrap" }}>
            <div><div className="mono dim" style={{ fontSize: 10, letterSpacing: ".2em", marginBottom: 7 }}>SCOPE</div><ScopeTag scope={prior.scope} /></div>
            <div><div className="mono dim" style={{ fontSize: 10, letterSpacing: ".2em", marginBottom: 7 }}>CONFIDENCE</div><Confidence level={prior.confidence} showLabel /></div>
            <div><div className="mono dim" style={{ fontSize: 10, letterSpacing: ".2em", marginBottom: 7 }}>ORIGIN</div><OriginTag origin={prior.origin} /></div>
          </div>
          ...
          <div className="panel pad" style={{ background: "rgba(45,212,191,.04)", borderColor: "rgba(45,212,191,.14)", marginBottom: 18 }}>
            ...
            <div style={{ fontSize: 14, lineHeight: 1.6, color: "var(--text-2)", textWrap: "pretty" }}>{prior.rationale}</div>
          </div>

          <div style={{ marginBottom: 8 }} className="mono dim"><span style={{ fontSize: 10, letterSpacing: ".2em" }}>ORIGIN NOTE</span></div>
          <div className="muted" style={{ fontSize: 12.5, marginBottom: 22 }}>{ORIGIN_DESC[prior.origin]}</div>

          {prior.triage && prior.triage !== "none" && (
            <div className="panel pad" style={{ marginBottom: 22 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 9 }}><TriageTag triage={prior.triage} /><span className="mono dim" style={{ fontSize: 10, letterSpacing: ".14em" }}>ADVISORY ONLY · A HUMAN DECIDES</span></div>
              <div className="muted" style={{ fontSize: 13, lineHeight: 1.55 }}>{prior.triage_reason}</div>
            </div>
          )}

          <div className="mono dim" style={{ fontSize: 10, letterSpacing: ".2em", marginBottom: 12 }}>SOURCE REFS</div>
          <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 30 }}>
            {prior.source_refs && prior.source_refs.length ? prior.source_refs.map((r, i) => (
          ...
          <div className="mono dim" style={{ fontSize: 10.5, letterSpacing: ".12em", marginBottom: 18 }}>
            created {timeAgo(prior.created_at)} · updated {timeAgo(prior.updated_at)} · <span style={{ color: "var(--dim)" }}>{prior.id}</span>
          </div>

          {prior.status === "candidate" && (onApprove || onReject) && (
            <div style={{ display: "flex", gap: 12, position: "sticky", bottom: 0 }}>
              <button className="btn primary lg" style={{ flex: 1 }} disabled={busy} onClick={() => onApprove(prior)}><Icon name="check" size={16} />Approve → Canonical</button>
              <button className="btn danger lg" disabled={busy} onClick={() => onReject(prior)}><Icon name="x" size={16} />Reject</button>
            </div>
          )}
        </div>
      </aside>
...
Object.assign(window, {
  ...
  TriageTag, TriageMeta: TRIAGE_META, EffectTag, ScoreRing, PriorRow, PriorDrawer, SectionTitle, ToastHost, useToast,
});
