...
The tool ("Metatron") stores a company's coding conventions as structured **decisions** and serves them to AI coding agents over MCP. When an agent is about to work on some code, it calls one function with:
...
The function must return the **8 most relevant decisions** (default limit) for that situation. Each decision is a record with these fields relevant to ranking:

- `scope` — a path string the decision applies to, e.g. `src/utils/i18n` (empty string = a global decision that applies everywhere)
...
Only `status == CANONICAL` (human-approved) decisions are served. A repo may have hundreds to low-thousands of canonical decisions. There are **no embeddings** — ranking is lexical only (a deliberate current constraint; embeddings may come later). Latency matters: this runs synchronously on every agent step.
...
## The algorithm (current implementation, Python)
    ...
    _STOPWORDS = {
        ...
        # instruction/filler language (common in task phrasing, rare in decisions,
        # so idf-over-decisions wrongly rates it HIGH):
    ...
    def _build_idf(decisions):
        # inverse document frequency over THIS repo's canonical decisions only
        n = len(decisions)
        df = {}
        for p in decisions:
    ...
    def _keyword_score(decision, query_tokens, idf):
        overlap = _tokens(decision.pattern + " " + decision.rationale) & query_tokens
        return sum(idf.get(tok, 0.0) for tok in overlap)

    def _pair_scope(decision_scope, area):
        # count shared leading path segments between the decision's scope and one area path
        ...
        if shared == len(target):                        return 2.0 + shared  # decision INSIDE the area
        if shared == len(scope):                         return float(shared) # decision is an ANCESTOR (closer = higher)
        return 0.0                                                     # siblings: share a parent then diverge

    def _scope_weight(decision_scope, area_paths):
        if decision_scope == "": return 1.0                 # global decision, weak
        return max((_pair_scope(decision_scope, a) for a in area_paths), default=0.0)

    def _relevance(decision, area_paths, query_tokens, idf):
        scope = _scope_weight(decision.scope, area_paths)
        keywords = _keyword_score(decision, query_tokens, idf)
        if scope == 0 and keywords == 0:
            return 0.0                                   # no signal -> filtered out
        return scope * _SCOPE_SCALE + keywords + _CONFIDENCE_WEIGHT[decision.confidence] * _CONF_SCALE

    def get_decisions_for_context(store, repo, file_path_or_area, task_description, limit=8):
        decisions = store.list(repo=repo, status=CANONICAL)
        ...
        # strongest pure task-keyword matches, so a relevant decision scoped OUTSIDE the
        # named area isn't crowded out by a directory full of generic same-scope decisions.
...
- **Scope is the strong decision**: the agent named a path, so decisions scoped exactly at / inside that path should usually win. Scope is scaled (`×10`) so an exact 3-segment match (`3.0 + 3 = 6` → `60`) dominates keyword scores, which are typically single digits.
- **Keywords are the tie-breaker AND an escape hatch**: among same-scope decisions, idf-weighted task-keyword overlap decides; and the *reserved slots* let a strongly task-relevant decision from a different directory break in.
- **idf is computed over the repo's own decisions** to avoid a hand-maintained stopword list — boilerplate ("shared", "module") gets near-zero weight automatically.
- **Stemming + stopwords** were added because (a) the task says "owner" while the decision says "ownership"; (b) instruction filler like "change"/"find"/"free" is rare in decisions so idf rated it *high*, letting coincidental matches win the reserved slots.
...
- The corpus DID contain the right decisions: an ownership/auth convention (`src/utils/ownership`, "Clerk auth") and a link convention (`src/utils/i18n`, "build internal links via ./urls helper instead of hardcoding URLs"). Both live OUTSIDE `ApplicationPage`.
- Before the reserved-slots + stemming + token-cleaning changes, the function returned 8 generic `ApplicationPage`-structure decisions and none of the relevant ones (ownership scored 0.0; link convention ranked ~#258).
- After the changes, the reserved slots correctly surface the `my-dashboard` Clerk-gating decision and the `blog/write` auth decision. BUT: the link-convention decision STILL doesn't surface — it says "url/links" while the task says "href" (lexical mismatch). And one reserved slot is wasted on a `docs/specs` decision that merely shares the verb "write".
...
2. **Failure modes.** Construct concrete query + corpus scenarios where this returns bad results (false positives that crowd out, or true positives it drops). Pay attention to: many same-scope decisions, multi-path areas, global (empty-scope) decisions, very small vs very large corpora, and the `confidence*0.1` term (does it ever matter?).
3. **The idf-over-decisions-only choice.** What are the consequences of computing idf only over decisions (not over query language)? The stopword list is the patch for it — is that sustainable, or is there a more principled fix?
