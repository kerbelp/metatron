# Design: prior helpfulness rating → auto-weighted ranking
...
Let coding agents rate each served prior **1–10** for how helpful it actually was,
then use those ratings to **automatically reorder which canonical priors get served
first** — so genuinely helpful priors rise and misleading ones sink over time — and
to surface a **leaderboard** that doubles as a human-curation queue for the priors
...
- **No prior self-promotes.** Ratings never promote a candidate to canonical, never
  un-reject, and never auto-demote a prior *out* of canonical. Crossing the canonical
  boundary stays 100% human-gated.
- **Auto-weighting only reorders *within* a scope tier.** Helpfulness adjusts the
  serve-time *order* among already-canonical priors that are already equally relevant
  by scope. It can push a misleading prior below the serve `limit` (so it stops being
  served) — but the prior still exists and removing it stays a human action.
- **"Helpful here ≠ better everywhere"** (the caution in `future-features.md` B): we
  aggregate a *global* per-prior score but apply it as a **bounded** term that only
  breaks ties *within a relevance tier*. A globally-loved prior can win among equally
  relevant peers; it can never jump the scope/keyword relevance gate to crowd out a
  more relevant prior. This is the structural mitigation for that caution.
- **No eval harness (item A) yet.** C's stated prerequisite is a trustworthy quality
  ...
  shrinkage so a handful of ratings can't swing a prior, and (d) the leaderboard's
  human review queue for low-scoring priors.

`CLAUDE.md` is updated to record that Metatron now has a **partial self-learning
loop**: usage ratings auto-weight serve ordering among canonical priors, while every
...
- **Wire format stays index-based.** `submit_feedback` gains `ratings: {index: score}`
  (keys are 1-based indices into the named query's served priors; scores 1–10).
  Agents never echo UUIDs — the service resolves indices → prior ids locally with the
...
- **Event model:** add `ratings: dict[str, int]` to `Event`, stored keyed by
  **prior_id** (the resolved form). Persisted as a new JSON column `ratings` on the
...
repo's FEEDBACK events, it produces `{prior_id: HelpfulnessScore}` where
...
- **Shrinkage toward neutral** — `score = (Σ wᵢ·rᵢ + k·NEUTRAL) / (Σ wᵢ + k)` with
  `NEUTRAL = 5.5` and pseudo-count `k = 3`. One rave can't crown a prior and one pan
  can't tank it; a *sustained* pattern moves the score.
- Output reused by both the serve path (ranking) and the UI (leaderboard) — one
  source of truth for "how helpful is this prior."

Tuning knobs (`HALF_LIFE`, `k`, `NEUTRAL`) live as named module constants.

## Ranking integration

- `get_priors_for_context` gains an optional `helpfulness: dict[str, float]` argument
  (prior_id → centered score). The service stays pure and testable; the **MCP server**
...
- **Guardrail (enforced by construction):** tiers (on-scope-topical → cross-scope-
  ...
  reorders entries *inside* a tier. It cannot move a prior across tiers, so it can
...
- Two ranked lists over canonical priors:
  - **Most helpful** — highest scores (the priors carrying their weight).
  - **Misleading / review** — lowest scores among priors with enough ratings to
    trust; each row carries the existing **Reject / Edit** actions, so the leaderboard
    is also the human-curation queue for priors the data says are hurting.
- Each row: pattern, scope, score (1–10), rating count, and the ranking effect
  ...
  screens (`N priors flagged for review`).

## Agent guidance

- `submit_feedback`'s tool docstring asks agents to rate each served prior 1–10 by
...
- Ranking: a higher-rated peer outranks a lower-rated peer *within* a tier; a loved
  off-scope prior still cannot jump a scope tier.
