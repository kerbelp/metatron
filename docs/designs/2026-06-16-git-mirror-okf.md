...
- **Date:** 2026-06-16
- **Status:** draft
- **Surface:** new `metatron sync` / `metatron import` CLI commands, a new
...
- **Machine-derived fields never round-trip.** `confidence`, `feedback_score`,
  `keywords`, and timestamps are computed by Metatron; the mirror shows them
  read-only and `import` ignores any human change to them.

## Architecture overview

```
            metatron sync  (DB -> files)
   SQLite  ───────────────────────────────►  metatron/candidate/*.md
 (source of                                   metatron/decisions/*.md
   truth)   ◄───────────────────────────────  (git-tracked mirror = OKF bundle)
            metatron import (files -> DB)
```

- **Export (`metatron sync`)** is the steady-state direction: it writes the
...
- **Import (`metatron import`)** is the human-edit direction: it reads the mirror
...
<!-- feedback: read-only, regenerated by `metatron sync` -->
## Feedback summary
...
| `confidence` | machine | ignored (warn if changed) |
| `feedback_score` | machine | ignored (warn if changed) |
...
last N feedback rationales — regenerated on every `metatron sync`. The raw
...
- **`metatron sync`** — full deterministic export of all `candidate` and
  `canonical` decisions to the mirror. Idempotent: re-running with no DB change
  produces no diff.
- **`metatron import`** — reads the mirror, computes per-decision diffs against
...
- `metatron sync --okf` (or an equivalent export flag) writes/validates the
...
- **`metatron/cli.py`** — `sync` / `import` subcommands (and `--okf`).
