...
#   - AGENTS.md gets a "read the context/ files first" block (not "query the MCP tool"),
#     appended to any existing AGENTS.md, never overwriting it.
...
if [[ ! -f "$SKILL_SRC/context-okf-llm-ingest/SKILL.md" || ! -f "$SKILL_SRC/context-okf-promote-candidates/SKILL.md" ]]; then
...
# Appends a managed block between METATRON markers to an AGENTS.md, once. Existing
...
if [[ "$SKILL_SRC" -ef "$ROO_SKILLS" ]]; then
  ...
  mkdir -p "$ROO_SKILLS"
  for skill in context-okf-llm-ingest context-okf-promote-candidates; do
    rm -rf "${ROO_SKILLS:?}/$skill"
    cp -R "$SKILL_SRC/$skill" "$ROO_SKILLS/"
  done
  echo "  copied OKF skills to $ROO_SKILLS (context-okf-llm-ingest, context-okf-promote-candidates)"
fi
...
<!-- METATRON:END -->'
add_claude_block "$WORKSPACE_ROOT/AGENTS.md" "$ROOT_BLOCK"

# Per-app block: only when the target is a subdirectory (monorepo app). Agents
# read a nested AGENTS.md when working in that subtree, so the app-specific pointer
...
<!-- METATRON:END -->'
  add_claude_block "$TARGET/AGENTS.md" "$APP_BLOCK"
fi

echo
echo "Done (files-first — no MCP, no hooks). Next steps:"
echo "  - Author candidates into $KB/candidate/ (see .roo/skills/context-okf-llm-ingest)."
echo "  - Promote reviewed ones with a human-approved 'git mv' to context/decisions/."
echo "  - Reconnect Roo / Claude Code so it picks up the new .roo rule and AGENTS.md."
